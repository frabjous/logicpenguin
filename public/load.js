...
LP.embed = async function(opts) {
  ...
  if ("cheat" in options && (options.cheat === false)) {
    options.noshowanswer = true;
  }
...
LP.makeProblems = async function(parentid, exerciseinfo,
  ...
  for (let probsetnum = 0; probsetnum < exerciseinfo.problemsets.length;
    ...
    for (const problem of probset) {
      ...
      if (probsetinfo?.options?.noshowanswer) {
        problemelem.noshowanswer = true;
      }
...
LP.sampleProblem = async function(opts) {
  let {
    parentid,
    problemtype,
    problem,
