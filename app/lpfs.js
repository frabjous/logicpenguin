...
lpfs.loadfile = function(filename) {
  let rv = null;
  try {
    rv = fs.readFileSync(filename, {encoding: "utf8", flag: "r"});
  } catch(err) {
    return null;
  }
  return rv;
...
// saves contents with a filename (generic)
lpfs.savefile = function(filename, contents) {
  try {
    fs.writeFileSync(filename, contents, {encoding: 'utf8', mode: 0o644});
  } catch(err) {
    return false;
  }
  return true;
